...
- Editor de estadísticas (`CharacterForm` en modo `estadistica`, tecla `U`) reestilizado como el panel del inventario: contenedor `form-content` con fondo degradado oscuro, borde y acentos usando el tema de clase del personaje (`inventory-class-*` + variables `--inventory-accent`, reutilizando `getClassThemeKey` de `inventoryUtils.js`), kicker de cabecera estilo inventario, filas de atributos como ranuras (borde/accento por clase) y botones +/auto distribuidos alineados al tema.
