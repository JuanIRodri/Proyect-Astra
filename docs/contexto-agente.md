...
- El editor de stats es navegable por teclado (`StatsFields.jsx`): `W/S` o `↑/↓` mueven la fila seleccionada (clase `is-focused` con acento), `A/D`, `←/→` o `+/-` asignan/quitan puntos, `R` resetea las cinco stats a 10, `T` auto-distribuye. Desde la última fila, `S`/`↓` pasa a la zona de acciones y enfoca "Guardar" (vía `submitRef`); `W`/`↑` vuelve a las stats. En la zona de acciones el keydown de stats no intercepta Enter, para que el formulario se envíe (guarda y cierra el editor).
