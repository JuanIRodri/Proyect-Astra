...
- [x] Diseñar y cargar un mapa de prueba exportado desde Tiled: la escena carga `frontend/public/maps/mapa-prueba.tmj` (Tiled JSON, 36×22 tiles, **orientación isométrica** 48×24) con su tileset `frontend/public/tiles/placeholder-v5.png` (placeholder: diamantes 48×24 de pasto, caminos de tierra y rocas) renderizado como capas Phaser Tilemap (`suelo` + `caminos` + `obstaculos`). **Vista isométrica real**: `game/isometric.js` provee la proyección (`isoProject`/`isoUnproject`/`isoWorldBounds`, equivalencia `(u−v)·24+24, (u+v)·12+12`, el **centro del diamante**, alineada con el ancla de render que usa Phaser: dibuja cada tile con el centro del frame en `pixelX + halfW, pixelY + halfH`) usada por collision, fichas, decoración, overlay de cuadrícula, minimapa y mapa `M` (que ahora dibujan el mundo en forma de diamante); las posiciones del grupo se guardan/emiten en tiles `(u, v)`. Fichas y árboles mantienen el volumen 2.5D con orden por `y` (`WORLD_DEPTH_BASE`). El suelo es **uniforme** (un solo tono de pasto para todos los tiles, con texturita fina: espejos/matas/piedritas) y cada tile está **recortado a su rombo con esquinas transparentes** (`PNG` RGBA), de modo que los rombos calzan exacto entre sí sin solaparse ni sobrescribir a los vecinos (el piso lo cubre la capa `suelo` y las rocas ya no "pisan" a las de alrededor), con borde del camino fundido al pasto; para el render se usa zoom entero + `roundPixels` (sin costuras entre rombos por píxeles fraccionarios). Para el mapa real: exportar Tiled isométrico (una celda = 48×24). Confirmado en el navegador.
- [x] Leer capas de colision desde un mapa Tiled: `game/collision.js` centraliza los objetos bloqueados (`setBlockedTiles`/`getBlockedTiles`/`isTileBlocked`/`canOccupy`) alimentados por la escena desde la capa `obstaculos` del `.tmj` + la decoración (`DECOR_DATA`); `movement.js` y `getSafePositions` (App.jsx) lo consumen. El minimapa y mapa `M` dibujan los tiles bloqueados con esa misma fuente (reemplazando `WALL_TILES`). **Hitbox por objeto 3D**: cada objeto tiene su propia elipse de colisión en píxeles de pantalla (`OBJECT_RADII`: árbol 10×9, arbusto 13×11, roca 14×9, calzando con el cuerpo visual) en vez del círculo único de tolken; `canOccupy` rechaza si el centro del personaje cae dentro de la elipse proyectada del objeto, manteniendo la comprobación de límites del mundo. El tileset `placeholder-v5.png` es arte procedural **recortado al rombo** (PNG RGBA con esquinas transparentes; sin relieve de loseta ni solape entre tiles) con rocas como canto rodado de 3 caras (contorno, lateral, superior con brillo y sombra). Confirmado en el navegador.
